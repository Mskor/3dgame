...
public class Render3D extends Render{
	...
	public void renderBounds(GameConveyor game) {
		...
		double walkingShake = Controller.walking ? Math.sin(game.time / 6.0) * 0.5
				: 0;

		for (int y = 0; y < height; y++) {
			double ceiling = (y - height / 2.0) / height;

			double z = (floorPosition + game.controls.y + walkingShake)
					/ ceiling;

			if (ceiling < 0) {
				z = (ceilingPosition - game.controls.y - walkingShake)
						/ ceiling;
			}
			...
			for (int x = 0; x < width; x++) {
				...
				zBuffer[x + y * width] = z;
				pixels[x + y * width] = floorTexCache.pixels[(xPix & 7)
						+ (yPix & 7) * 8];
			}
		}
